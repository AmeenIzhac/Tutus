...
proj _ p g | not (p `elem` roles g) = SEnd
proj res p g@(GComm q r v ks) 
  | p == q = 
    case mapMaybe f ks of
      [] -> error ("sends must have at least one continuation" ++ show g)
      ks' -> SSend r v ks'
  where
    f k | isCrashBranch k = Nothing
        | otherwise = case proj res p (cont k) of
          gi' -> Just (Choice (label k) (payload k) gi')
proj res p (GComm q r v ks)
  | p == r = SRecv q v (map f ks)
    where
      f k = Choice (label k) (payload k) (proj res p (cont k))
proj res p (GComm q r _ ks) 
  | p /= q && p /= r = fullMerge p (map (proj res p . cont) ks)
...
fullMerge :: (Annot a, Show a) => Role -> [S a] -> S a
fullMerge p' [] = error ("Cannot merge an empty list of continuations in projection on " ++ show p')
fullMerge p' ks = foldr1 merge' ks 
  where
    ...
    merge' (SRecv p v xs) (SRecv q w ys)
      ...
      where
        ...
        f ys' x = 
          let y = find (label x) ys' 
            in if payload x /= payload y
              then error ("payload type differs: " ++ show (x,y))
              else Choice (label x) (payload x) (merge' (cont x) (cont y))
    merge' (SSend p v xs) (SSend q w ys) 
      | p == q && prop =
        SSend p (mergeAnnot v w) zs 
      where
        assoc x = ((show . label) x, payload x)
    ...
    merge' s t = error ("undefined case in merge on " ++ show p' ++ ":\n" ++ show (s,t))

projAllRoles :: (Annot a, Show a) => G a -> [(Role, S a)]
projAllRoles g = let rs = filter reliable (participants g) in
